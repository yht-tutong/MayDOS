...
current_path = os.path.dirname(os.path.abspath(__file__))[:-11]
...
def ListUsers():
    ...
    try:
        ...
        os.makedirs(os.path.abspath("./Users"))
    finally:
        for i in os.listdir(os.path.abspath(os.path.dirname(os.path.abspath(__file__)) + "../../../Users")):
            # 检查文件是否以".txt"结尾
            if i.endswith(".txt"):
...
def ls(cmd, sargs):
    ...
    for i in temp:
        if i.startswith(".") and not "a" in sargs:
            continue
